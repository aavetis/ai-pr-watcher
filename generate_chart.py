...
def generate_chart(csv_file=None):
    ...
    # Add bar value labels with smaller font size
    for i, (cop_total, cop_merged, cod_total, cod_merged, cur_total, cur_merged, dev_total, dev_merged) in enumerate(
        zip(df["copilot_total"], df["copilot_merged"], df["codex_total"], df["codex_merged"], 
            df["cursor_total"], df["cursor_merged"], df["devin_total"], df["devin_merged"])
    ):
        # Copilot bars
        if cop_total > 0:
